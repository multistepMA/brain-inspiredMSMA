...
def main(path, outpath):
    ...
    for sub in range(len(sig_addpath)):
        print(sig_addpath[sub][:-4].split('\\')[-1])
        ...
        for _vf in tqdm(range(_mov_first_zone, _mov_last_zone, 250)):
            ...
            if len(_sub_sig[_class_first_zone:(_class_first_zone + 250 * 30)]) > ((250 * 30) - 10):
                class_dir = os.path.join(outpath, 'Classification', sig_addpath[sub][:-4].split('\\')[-1], str(_arr_num_pos)[1:-3])
                pred_dir = os.path.join(outpath, 'Pred', sig_addpath[sub][:-4].split('\\')[-1], str(_arr_num_pos)[1:-3])
                os.makedirs(class_dir, exist_ok=True)
                os.makedirs(pred_dir, exist_ok=True)

                class_csv_path = os.path.join(class_dir, '{}_{}.csv'.format(sig_addpath[sub][:-4].split('\\')[-1], _class_first_zone))
                pred_csv_path = os.path.join(pred_dir, '{}_{}{}.csv'.format(sig_addpath[sub][:-4].split('\\')[-1], _class_first_zone, str(np.array([_pred_output1, _pred_output2, _pred_output3, _pred_output4, _pred_output5]))))
