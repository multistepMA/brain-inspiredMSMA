...
def main(path, outpath):
    ...
    for sub in range(22, len(sig_addpath)):
        data_id = sig_addpath[sub][:-4].split('\\')[-1]
        print(data_id)
        record = wfdb.rdrecord(sig_addpath[sub][:-4]).p_signal[:, 0]

        _vfonset = _calc_vfonset(_vfonset_info, sig_addpath[sub][:-4].split('\\')[-1])
        _sub_diff, _sub_pos, _sub_arrhythmia_type = _read_ann(path + sig_addpath[sub][:-4].split('\\')[-1], len(record), _vfonset)
        ...
        if _vfonset > 0 and _vfonset_info['VF Onset'][sub] != 'remove':
            for vf_index, _vf in enumerate(tqdm(range(_mov_first_zone, _mov_last_zone, 250))):

                _pred_vf_zone = (_vfonset + _vf) - (60*60*250)
                ...
                _pred_forth_zone = _pred_vf_zone - (120*60*250)            
                _pred_fifth_zone = _pred_vf_zone - (60*60*250)         

                if (_pred_fifth_zone + (60*60*250)) < len(record) and (_pred_forth_zone - (10*60*250)) > 0 and _pred_first_zone > (60*60*250):
                    pred_zones = [_pred_first_zone, _pred_second_zone, _pred_third_zone, _pred_forth_zone, _pred_fifth_zone]

                    for i, zone in enumerate(pred_zones):
                        class_start_zone = zone - (10*60*250)

                        if class_start_zone < 0 or (zone + (60*60*250)) > len(_sub_sig) or (zone + (60*60*250) - class_start_zone) != (70*60*250):
                            continue

                        arr_num_pos, _, _ = get_labels_start_end_time(np.array(_step_label[class_start_zone:(class_start_zone + 250*30)]))
                        pred_output = _pred_zone_info(_step_label, zone, zone + (60*60*250), labeldict)

                        directory = f"{outpath}Classification_5hr/{data_id}/VF/{str(arr_num_pos)[1:-3]}"
                        if not os.path.exists(directory):
                            os.makedirs(directory)
                        file_name = f"{data_id}_{vf_index+1}_{class_start_zone}_{pred_output}"

                        pd.DataFrame([_sub_sig[class_start_zone:(class_start_zone + 250*30)], _step_label[class_start_zone:(class_start_zone + 250*30)]]).transpose().to_csv(f"{directory}/{file_name}.csv")

        elif _vfonset == 0 and _vfonset_info['VF Onset'][sub] == '(no VF)':
            for no_vf_index, _no_vf in enumerate(tqdm(range(_begin, len(record), 1250))):
                _class_first_zone = _no_vf

                _pred_no_first_zone = _class_first_zone + (30*250) + (60*60*250)
                _pred_no_second_zone = _pred_no_first_zone + (60*60*250)
                _pred_no_third_zone = _pred_no_second_zone + (60*60*250)
                _pred_no_forth_zone = _pred_no_third_zone + (60*60*250)
                _pred_no_fifth_zone = _pred_no_forth_zone + (60*60*250)

                if (_pred_no_forth_zone + (30*60*250)) < len(record) and _pred_no_fifth_zone + (60*60*250) < len(record) and _pred_no_first_zone > (60*60*250):
                    pred_zones = [_pred_no_first_zone, _pred_no_second_zone, _pred_no_third_zone, _pred_no_forth_zone, _pred_no_fifth_zone]

                    for i, zone in enumerate(pred_zones):
                        class_start_zone = zone - (30*250) - (60*60*250)

                        if class_start_zone < 0 or (zone + (60*60*250)) > len(_sub_sig) or (zone + (60*60*250) - class_start_zone) != ((60*60*250) + (30*250) + (60*60*250)):
                            continue

                        arr_num_pos, _, _ = get_labels_start_end_time(np.array(_step_label[class_start_zone:(class_start_zone + 250*30)]))
                        pred_output = _pred_zone_info(_step_label, zone, zone + (60*60*250), labeldict)

                        directory = f"{outpath}Classification_5hr/{data_id}/NoVF/{str(arr_num_pos)[1:-3]}"
                        if not os.path.exists(directory):
                            os.makedirs(directory)
                        file_name = f"{data_id}_{no_vf_index+1}_{class_start_zone}_{pred_output}"
                        pd.DataFrame([_sub_sig[class_start_zone:(class_start_zone + 250*30)], _step_label[class_start_zone:(class_start_zone + 250*30)]]).transpose().to_csv(f"{directory}/{file_name}.csv")
        ...
        for no_vf_index, _no_vf in enumerate(tqdm(range(_begin, _pred_first_zone_no, 1250))):
            ...
            if (_pred_no_fifth_zone + (60*60*250)) < len(record) and _pred_no_first_zone > (60*60*250):
                pred_zones = [_pred_no_first_zone, _pred_no_second_zone, _pred_no_third_zone, _pred_no_forth_zone, _pred_no_fifth_zone]
                ...
                for i, zone in enumerate(pred_zones):
                    ...
                    directory = f"{outpath}Classification_5hr/{data_id}/NoVF_2/{str(arr_num_pos)[1:-3]}"
                    if not os.path.exists(directory):
                        os.makedirs(directory)
                    file_name = f"{data_id}_{no_vf_index+1}_{class_start_zone}_{pred_output}"
                    pd.DataFrame([_sub_sig[class_start_zone:(class_start_zone + 250*30)], _step_label[class_start_zone:(class_start_zone + 250*30)]]).transpose().to_csv(f"{directory}/{file_name}.csv")
